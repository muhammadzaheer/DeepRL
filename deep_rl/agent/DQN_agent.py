...
class DQNTileAgent(BaseAgent):
    ...
    def step(self):
        ...
        if self.total_steps > self.config.exploration_steps:
            ...
            #
            # states = self.config.state_normalizer(states)
            # ts = self.tiles_rep.get_representation(states[0])
            #
            # next_states = self.config.state_normalizer(next_states)
            # tns = self.tiles_rep.get_representation(next_states[0])

            ts, actions, rewards, tns, terminals = experiences

            q_next = self.target_network(tns[0]).detach()
            q_next = q_next.max()
            terminals = tensor(terminals)
            rewards = tensor(rewards)
            q_next = self.config.discount * q_next * (1 - terminals)
